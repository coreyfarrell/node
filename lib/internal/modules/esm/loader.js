...
class Loader {
  ...
  async getFormat(url) {
    ...
    if (!defaultLoaderWorker) {
      // Maybe this should instead happen after resolve?
      let parsed;
      try {
        parsed = new URL(url);
      } catch {
...
exports.initUserLoaders = (bottomLoader) => {
  if (!bottomLoader) {
    bottomLoaderRPC = require('internal/modules/esm/default_loader_worker');
    return;
  }
  defaultLoaderWorker = false;
