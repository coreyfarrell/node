...
function defaultFetchSource(url) {
...
function getSource(url, fetchSource) {
  if (fetchSource) {
    return fetchSource(url, defaultFetchSource);
  }

  return defaultFetchSource(url);
}
...
translators.set('module', async function moduleStrategy(
  url,
  isMain,
  fetchSource
) {
  const source = `${await getSource(url, fetchSource)}`;
...
translators.set('json', async function jsonStrategy(url, isMain, fetchSource) {
  ...
  const content = `${await getSource(url, fetchSource)}`;
  if (pathname) {
...
translators.set('wasm', async function(url, isMain, fetchSource) {
  const buffer = await getSource(url, fetchSource);
